...
export function paste_handler(
    ...
    upload_pasted_file: (pasted_file: File) => void,
    min_paste_size_for_file_treatment: number,
): void {
    ...
    if (clipboardData.getData) {
        ...
        const paste_text_length = paste_text.length;
        ...
        const is_paste_large_enough_for_file =
            paste_text_length >= min_paste_size_for_file_treatment;
        ...
        if (
            ...
            upload_pasted_file(pasted_file);
...
export function initialize({
    min_paste_size_for_file_treatment,
    upload_pasted_file,
}: {
    min_paste_size_for_file_treatment: number;
    upload_pasted_file: (pasted_file: File) => void;
    ...
    $<HTMLTextAreaElement>("textarea#compose-textarea").on(
        ...
        function (this: HTMLTextAreaElement, event: JQuery.TriggeredEvent) {
            paste_handler.call(this, event, upload_pasted_file, min_paste_size_for_file_treatment);
        },
    );
